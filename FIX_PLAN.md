...
| 4.6 ACOLITE on a real scene | **Done for one scene** (2026-09-20): real L1C downloaded, ACOLITE DSF run, output converted to the band contract and compared with Sen2Cor (`reports/real/acolite_dry_run.json`, `AQUA_SENSE_PROJECT_PLAN.md` section 15). Five wrapper problems found and fixed. No model was retrained on ACOLITE output; results still use Sen2Cor. |
| 4.7 C2RCC on a real scene | **Open; needs ESA SNAP, which this project never had.** The pitch must not claim dual-tier correction. |
...
- [x] **4.6 Atmospheric correction is not connected to anything (ACOLITE part verified on a real scene 2026-09-20; C2RCC not).** `correct_scene` returns file paths; nothing converts ACOLITE/C2RCC outputs to the `{site}_{sensor}_{date}.tif` contract, so the downstream pipeline only ever sees the Earth Engine fallback. "Routing" is a static site-type rule, not the per-pixel dynamic routing the pitch describes.
