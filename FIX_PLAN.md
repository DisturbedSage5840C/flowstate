...
- [ ] **0.1 Fresh install must work.** `requirements.txt` lacks `pyyaml` and `streamlit-folium`. `plotly>=5.18` resolves to 7.x, where `px.scatter_mapbox` no longer exists, so [streamlit_app.py:393](src/app/streamlit_app.py#L393) crashes on the map tab when `streamlit-folium` is absent.
...
- [ ] **0.2 Raster map layer crashes.** [raster_layers.py:45](src/app/raster_layers.py#L45) calls `cm.get_cmap`, removed in matplotlib 3.9 (reproduced on 3.11).
...
- [ ] **0.3 Remove every hardcoded result from the dashboard** ([streamlit_app.py](src/app/streamlit_app.py)).
...
- [ ] **0.4 Dashboard must show model output, not dataset labels.** `run_model_predict` ([L183](src/app/streamlit_app.py#L183)) is never called. `get_predictions` displays the parquet's label columns as "predictions", and the model radio does nothing.
...
- [ ] **0.5 Add a persistent banner** while data is synthetic/proxy: "Demonstration data — not measurements" (see D1).

## Phase 1 — Make the numbers honest

- [ ] **1.1 Labels are exact formulas of the features (leakage).** Verified: `chl_a == 10^(1.35·ndci+1.58)` (max diff 3e-5), `turbidity == Nechad(B4,B8)` (diff 0.0). A site-grouped gradient-boosting model gets R² 0.998 / 0.991 / 0.891 (Chl-a / turbidity / DO). Real SHAP confirms the models just recover the formula (Chl-a: `ndci`; turbidity: `B4`).
...
- [ ] **1.5 DL evaluation does not use spatial CV.** [train_dl_model.py](scripts/train_dl_model.py) holds out the last three sites alphabetically (Ulsoor, Varthur, Yamuna) — this splits the Bengaluru cluster (Bellandur is in train) — and uses a random row split for validation. Per-type "river" is a single site. Reported: Chl-a R² 0.91, turbidity −1.34, DO −0.51 (worse than a constant), versus 0.999 / 1.0 / 0.96 for a plain gradient-boosting model on the same split.
...
- [ ] **1.6 One metrics format, one comparison table** written by one script and read by the dashboard.

## Phase 2 — Scientific correctness

- [ ] **2.1 DO surrogate is wrong.** [feature_engineering.py:150](src/features/feature_engineering.py#L150): `14.62 − 0.3898·T` gives 4.9 / 3.7 / 1.0 mg/L at 25 / 28 / 35 °C versus true saturation 8.3 / 7.8 / 7.0 (errors of −3.4 / −4.1 / −6.0). DO has the largest WQI weight.
...
- [ ] **2.2 WQI is degenerate and defined three ways.**
...
- [ ] **2.3 BOD is invented in the dashboard.** [streamlit_app.py:115](src/app/streamlit_app.py#L115) (`bod = turbidity·0.15 + …`) and the models do not predict BOD, but the WQI needs it.
...
- [ ] **2.4 Chl-a proxy formula.** `10^(1.35·NDCI+1.58)` is described as "Mishra & Mishra 2012, calibrated for Indian inland waters". It does not match the published NDCI polynomial (repo 96.6 vs 57.4 µg/L at NDCI 0.3), and the "Indian calibration" is unsupported.
...
- [ ] **2.6 2BDM/3BDM do not match the plan.** Plan: `2BDM = B5/B4`. Code: `1/B4 − 1/B5`. `3BDM` needs B6 (740 nm), which is not exported, so the code silently substitutes B8 (842 nm).
  Fix: implement the plan's formulas; export B6 or drop `bdm3` — no silent substitution.
- [ ] **2.7 MNDWI threshold per site.** Default 0.0 everywhere.
...
- [ ] **3.1 The "temporal" model has no temporal input (D3).** Dataset yields shape `(1, 12)`; `window_size` is stored but never used. Verified: attention weights are exactly 1.0 for every sample; conv kernels 3/5, BiLSTM and attention are no-ops, so the model is an MLP.
...
- [ ] **3.2 Targets are unscaled.** Chl-a mean ≈ 224 vs DO ≈ 2 with Huber δ = 1, so Chl-a dominates the loss.
  Fix: standardise (or log-transform) targets, invert in `predict`, weight the loss per target.
- [ ] **3.3 Silent bad inputs.** `AquaSenseDataset` fills a missing column with 0.0 ([dl_model.py:73](src/models/dl_model.py#L73)); a missing `temp_surface` becomes z ≈ −7.7 after scaling (verified). `predict()` returns garbage from untrained weights with only a warning when `.pt`/`.pkl` are absent (both are gitignored; verified outputs ≈ 0).
...
- [ ] **3.4 Single source of truth for the model interface.** `FEATURE_COLS`/`TARGET_COLS` are duplicated in `dl_model.py` and `xgboost_pipeline.py`. The old `assign_spatial_folds`/`get_fold_splits` contract in `spatial_cv.py` was replaced by `SpatialKFold`, and `SpatialKFold` hardcodes `["lat","lon"]` ignoring `lat_col`/`lon_col`. XGBoost clip bound for Chl-a is 500 while labels reach 668.
...
- [ ] **3.5 Spatial CV wording (D4).** The code uses its own KMeans site blocks, not the `spatial-kfold` library named in the plan/pitch. Update all wording, or wrap the library.
- [ ] **3.6 Lazy imports.** [src/models/__init__.py](src/models/__init__.py) eagerly imports xgboost, optuna and shap, so anything importing `src.models.*` (dashboard, DL script) pulls them all. Import lazily.
- [ ] **3.7 Model selection.** One script trains/evaluates both models on the same folds and writes the comparison table (feeds 0.3/1.6); winner chosen by out-of-fold RMSE.

## Phase 4 — Pipeline integration

- [ ] **4.1 Band contract.** Verified failures: river rasters (B2,B3,B4,B8 only) → `compute_all_features` raises `KeyError: 'B5'`; Landsat rasters (B3,B4,B5,B6) → `KeyError: 'B8'`. Worse, Landsat B5 is NIR and must never be treated as red-edge. Landsat has no red-edge band at all, so NDCI/Chl-a cannot come from Landsat.
...
- [ ] **4.2 Sensor codes.** The acquisition code emits `S2`/`LS`; the join only knows `S2`/`L8`/`L9`/`LANDSAT`, so `LS` silently gets a 3-day tolerance instead of 5 (verified).
  Fix: one shared sensor enum/constants module.
- [ ] **4.3 `temp_surface` has no producer.** Not exported by the raster pipeline, not produced by `compute_all_features`.
...
- [ ] **4.4 Missing script: rasters → training table.** Nothing turns exported `.tif` files into `train.parquet`.
...
- [ ] **4.5 Export size (unverified).** `gee.export_scene` uses `geemap.ee_export_image`. At 10 m, most river bounding boxes in [sites.yaml](config/sites.yaml) would exceed Earth Engine's direct-download size cap (estimated from bbox × bands × 4 bytes).
...
- [ ] **4.8 Dashboard ↔ raster integration.** The dashboard never imports `raster_layers`; the map shows one circle per site, not pixel maps. Upload-AOI only previews the polygon and then selects **all** sites (D5).
...
- [ ] **4.9 `predict_raster` band handling.** It raises on band-count mismatch but has no river/Landsat band lists; wire it to the band contract in 4.1.

## Phase 5 — Repo hygiene and tests

- [ ] **5.1 `.gitignore` regression.** The latest change removed `.venv/`, `.pytest_cache/`, `*.pyc` and `*.tif` (a `.venv` exists in the working directory and would show as thousands of untracked files) and added `data/processed/`, `data/ground_truth/`, which hide new files in folders that contain tracked demo data.
  Fix: restore the removed lines; decide which data files are tracked demo assets and un-ignore them.
- [ ] **5.2 Two synthetic-data generators.** [generate_synthetic_train.py](scripts/generate_synthetic_train.py) and [generate_synthetic_data.py](scripts/generate_synthetic_data.py) both write `data/processed/train.parquet`; README references both.
  Fix: keep one (the fixed one from 1.1); delete the other; update README and the error message in `train_models.py`.
- [ ] **5.3 Weak or vacuous tests.**
...
- [ ] **5.4 Dashboard smoke test.** Add an `AppTest`-based test that runs the app and asserts zero exceptions and that model output changes with the radio.
...
- [ ] River rule: "10 m bands only" contradicts NDCI needing B5; replace with the 4.1 contract.
- [ ] Landsat is not a Chl-a gap-filler (no red-edge band); state its actual role.
- [ ] Define the WQI scale and the separate best-use class exactly as implemented (2.2).
- [ ] State the label strategy and the disclosure wording (1.3, D1).
- [ ] Replace "spatial-kfold library" wording per D4.
- [ ] Pitch claims: "R² > 0.85" only if it comes from out-of-fold predictions on real data; SHAP statement must reflect what SHAP shows (NDCI dominates; B5 enters through NDCI).
- [ ] Mark status of each deliverable, and record which claims are backed by evidence.
