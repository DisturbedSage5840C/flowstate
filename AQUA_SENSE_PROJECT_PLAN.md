...
| **Pollution screening (primary deliverable)** | done; **BOD > 3 mg/L AUC 0.755** (precision 0.77 in the top 10 % vs a 28.8 % base rate), BOD > 6 AUC 0.777, CPCB polluted/acceptable AUC 0.730, DO < 4 AUC 0.720. Produces a ranked "inspect first" shortlist | `scripts/train_screening.py`, `reports/real/screening_metrics.json`, `reports/real/screening_shortlist.csv` |
| Per-target feature sets | done; measured by ablation on identical rows/folds — context features lift BOD (−0.011 → +0.121) and **hurt** DO and turbidity, so each target now has its own feature list; rainfall is in none of them | `src/models/schema.py`, `data/ground_truth/data_source_log.md` |
| Regression variants that work | station-level BOD ranking R² ≈ 0.20 / Spearman ≈ 0.39; turbidity anomaly vs a station's own mean R² ≈ 0.08 | `reports/real/metrics_summary.json` (`variants`) |
| Open-Meteo antecedent rainfall | fetched and tested; **adds nothing** (+0.001 on BOD, negative on DO/turbidity) — kept as data, excluded from every feature set | `src/data/weather.py` |
...
| R² > 0.85 | **not achieved and not achievable with this signal**; out-of-fold raw R² stays near zero even after recalibration and more data — this is a genuine ceiling of single-satellite-scene optical reflectance for chemistry parameters under honest spatial CV, not a bug. Rank correlation (Spearman 0.14-0.40) and log-scale R² are the honest regression numbers and both beat naive baselines. The project's headline is now the screening AUC instead, which is what the data supports | `reports/real/metrics_table.csv`, `reports/real/screening_metrics.json` |
| Why the ceiling is real (measured) | 85-100 % of each target's variance is *between* stations and site-blocked CV withholds it; an oracle knowing each station's own mean would reach R² 0.61/0.79/0.49. A learning curve is flat (screening AUC 0.743 at 25 % of stations → 0.755 at 100 %), so more extraction would not help | `data/ground_truth/data_source_log.md` §4 |
